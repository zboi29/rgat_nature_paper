...
This guide is written for editors, peer reviewers, and contributors who want a clear map from the
...
| --- | --- | --- |
| S1 (Sign invariance) | `RgatPaperProofs/Geometry/Basic.lean:39` | `sign_invariance` lemma. |
| S2 (Small-angle expansion) | `RgatPaperProofs/Geometry/SmallAngle.lean:177` (statement), `:182` (proof) | Includes explicit ε0 parameter. |
| S3 (Softmax stability) | `RgatPaperProofs/Gradients/Statements.lean:155` (statement), `RgatPaperProofs/Gradients/Proofs.lean:32` (proof) | Mean‑value/regularity machinery. |
| S4 (Bridge Theorem, head) | `RgatPaperProofs/Attention/Logits.lean:74` (statement block) | Softmax/logits + geometric → Euclidean limit. |
| S4 (Bridge Theorem, stack) | `RgatPaperProofs/Gradients/Statements.lean:110` (statement), `RgatPaperProofs/Gradients/Proofs.lean:113` (proof) | Explicit product constant; assumes Lip ≥ 1. |
| S5–S8 | `RgatPaperProofs/Gradients/Proofs.lean:257`, `:285`, `:274`, `:307` | Theorem S5, Corollary S6, Lemma S7, Corollary S8. |
| S9 (Gauge equivariance) | `RgatPaperProofs/Transformers/Statements.lean:33` (statement), `RgatPaperProofs/Gradients/Proofs.lean:364` (proof) | Statement + proof. |
...
| S13 (Depth accumulation) | `RgatPaperProofs/Gradients/Statements.lean:87` (statement), `RgatPaperProofs/Gradients/Proofs.lean:673` (proof) | Explicit remainder bound. |
| S14 (Corollary) | `RgatPaperProofs/Gradients/Statements.lean:131` (statement), `RgatPaperProofs/Gradients/Proofs.lean:731` (proof) | Stack‑level approximation + curvature clause. |

## How to navigate the Lean package

Start with the umbrellas:

- `RgatPaperProofs/Geometry.lean`
- `RgatPaperProofs/Attention.lean`
- `RgatPaperProofs/Transformers.lean`
- `RgatPaperProofs/Gradients.lean`

Then drill down:

- Geometry and small‑angle lemmas: `RgatPaperProofs/Geometry/*`
- Softmax/logits and head‑level Bridge Theorem: `RgatPaperProofs/Attention/*`
- Stack‑level bound + gradient statements: `RgatPaperProofs/Gradients/*`
