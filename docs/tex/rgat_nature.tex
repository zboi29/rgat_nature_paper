...
\title{Standard Attention as a Small-Angle Limit of Riemannian Geometric Algebra Transformers}
...
classical dot-product attention; yet,
at depth, many small-angle steps accumulate curvature through
Baker--Campbell--Hausdorff commutators \cite{hall2015lie}, providing a precise sense in which
...
performance studies are a separate empirical program.
Formal verification and reproducibility artifacts for this manuscript are
available in the repository release \cite{joseph2026rgatsoftware}.
\end{abstract}
...
\cite{vaswani2017attention,coifman2006diffusion}.
...
$\alpha_{ij}\propto\exp(-\|q_i-k_j\|^2/(2\tau))$ \cite{vaswani2017attention}.
...
enforce this regime during analysis \cite{hall2015lie}.
...
mathematically transparent \cite{berline1992heat}.
...
positive kernel, its spectrum lies in the unit disk. The right eigenvector at
$\lambda=1$ is the constant vector $\mathbf{1}$ ($P\mathbf{1}=\mathbf{1}$), and
the left eigenvector at $\lambda=1$ is a stationary distribution; strict
positivity of the kernel implies uniqueness in the finite-key setting. This
provides a spectral notion of diffusion time and mixing:
the leading eigenmodes capture low-frequency structure on $\Spin(3)$, while the
spectral gap controls the decay of high-frequency components. In this view,
attention weights encode a geometry-dependent low-pass filter, and the operator
family varies continuously with the rotor field.
...
\begin{equation}
  \ell_{\mathrm{GSM}}(u,v)=\frac{1}{\tau}u^\top v-\frac{1}{2\tau}\|v\|^2+C_i
  +O(\varepsilon^2).
\end{equation}
where $C_i$ is constant in $j$ within row $i$ (query-dependent) and cancels in
row-wise softmax. Therefore, row-wise softmax matches dot-product attention to $O(\varepsilon^2)$
under bounded logits (SI Lemmas~S2--S3 and SI Theorem~S4). This is the precise
...
$\|Q_i\|,\|K_j\|\le\varepsilon\le\varepsilon_0<\pi$ and $\tau\ge\tau_{\min}>0$.
Define $R(u):=\exp(u)$ (the rotor map from Lie-algebra coordinates), and
\begin{equation}
...
\begin{equation}
  \|\mathcal{F}_{\mathrm{RGAT}}-\mathcal{F}_{\mathrm{Transformer}}\|_{\infty,2}
...
$\|u_\ell\|\le\varepsilon$, with $L\varepsilon$ below the injectivity radius and
$L\varepsilon\le 1$.
...
on log-log axes; fitted slope $m \approx 1.988$ confirms $O(\varepsilon^2)$ scaling; correlation coefficient is $r^2 \approx 1.0$.
(b) Depth-accumulated error scales linearly with $L$ at fixed $\varepsilon = 0.1$;
inset shows BCH commutator curvature growth.
Code and seeds included in the software release \cite{joseph2026rgatsoftware};
experiment runs in ${\sim}15$\,s on CPU.}
...
curvature. The corresponding Lean and Python artifacts are released with the
codebase \cite{joseph2026rgatsoftware}.
...
\cite{bronstein2021geometric,brehmer2023geometric}.
...
We thank the open-source community for the libraries and tooling that make this work practical.
...
\bibitem{hall2015lie}
Hall, B. C. \emph{Lie Groups, Lie Algebras, and Representations: An Elementary Introduction}. 2nd edn (Springer, 2015).

\bibitem{casas2008bch}
Casas, F. and Murua, A. An efficient algorithm for computing the Baker--Campbell--Hausdorff series and some of its applications. \emph{arXiv preprint arXiv:0810.2656} (2008).

\bibitem{berline1992heat}
Berline, N., Getzler, E. and Vergne, M. \emph{Heat Kernels and Dirac Operators} (Springer, 1992).

\bibitem{joseph2026rgatsoftware}
Joseph, Z. N. RGAT: Riemannian Geometric-Algebra Transformers (Nature Paper) (v0.1.2). \emph{Zenodo} https://doi.org/10.5281/zenodo.18511210 (2026).
\end{thebibliography}
