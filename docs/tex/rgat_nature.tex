...
\textbf{Euclidean attention as a Gaussian kernel.} For standard attention with
queries $q_i$ and keys $k_j$, the logits $q_i^\top k_j/\tau$ can be rewritten as
\begin{equation}
  ...
  = -\frac{1}{2\tau}\|q_i-k_j\|^2
    ...
    + \frac{1}{2\tau}\|k_j\|^2.
\end{equation}
Row-wise softmax eliminates the $\|q_i\|^2$ term, so standard attention is
equivalent to a Gaussian kernel on Euclidean space up to key-dependent bias.
RGAT replaces the Euclidean distance with the intrinsic geodesic distance on
$\Spin(3)$, yielding a genuine heat-kernel operator on a curved manifold.
...
on log-log axes; fitted slope $m \approx 1.96$ confirms $O(\varepsilon^2)$ scaling.
(b) Depth-accumulated error scales linearly with $L$ at fixed $\varepsilon = 0.1$;
inset shows BCH commutator curvature growth.
Code and seeds included; experiment runs in ${\sim}7$\,s on CPU.}
...
Vaswani, A. \emph{et al.} Attention is all you need. (2017).
\bibitem{hestenes2015spacetime}
Hestenes, D. \emph{et al.} Spacetime Algebra (2015).
\bibitem{coifman2006diffusion}
Coifman, R. and Lafon, S. Diffusion maps. (2006).
\end{thebibliography}
