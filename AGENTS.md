# AGENTS.md — RGAT Paper Proofs

## Project Summary
This repo contains the code and verification artifacts for the **Riemannian Geometric-Algebra Transformer (RGAT)** paper. It formalizes that standard Transformer attention is a flat-geometry limit of a curved diffusion operator on the rotor manifold \(\Spin(3)\), and includes Lean 4 proofs plus Python validation/plotting.
...
- Key files:
  - `RgatPaperProofs/Geometry/Basic.lean` — \(\Spin(3)\) rotors + geodesic distance (SI definitions).
  - `RgatPaperProofs/Geometry/SmallAngle.lean` — Lemma S2 small-angle expansion.
  - `RgatPaperProofs/Attention/Logits.lean` — logits + S4 head-level bridge.
  - `RgatPaperProofs/Gradients/Proofs.lean` — S3, S10–S14 proofs and stack-level bridge clause.
...
- Edit: open `RgatPaperProofs/` in VS Code with the Lean 4 extension.
...
1. Check math in `docs/tex/si_rgat_paper.tex`.
